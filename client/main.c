 ...
 * and adapted for other academic purposes. 
*********************************************************/
...
/*********************************************
 * This function generates and publishes
 * random values for five different sensors:
 *
 * Temperature,
 * Humidity,
 * Wind Direction,
...
static int env_data_pub(int argc, char **argv)
{
    emcute_topic_t t;
    unsigned flags = EMCUTE_QOS_0;
    ...
    while (1){
      ...
      int humidity = random_uint32_range(0, 100);
      int wind_direction = random_uint32_range(0, 360);
      int wind_intensity = random_uint32_range(0, 100);
      int rain_height = random_uint32_range(0, 50);
      char env_data[5] = {temperature, humidity, wind_direction,
                        wind_intensity, rain_height};
      /* step 2: publish data */
      if (emcute_pub(&t, env_data, sizeof(env_data), flags) != EMCUTE_OK) {
          printf("error: unable to publish data to topic '%s [%i]'\n",
                  t.name, (int)t.id);
          return 1;
      }
...
static const shell_command_t shell_commands[] = {
    ...
    { "rand", "generate sensors random values", env_data_pub},
    { NULL, NULL, NULL }
};
